...
function! s:my_ft_molder() abort
    ...
    nmap <buffer> <nowait> t :<C-u>call <SID>molder_tab_open()<CR>
    nmap <buffer> <C-k> :<C-u>normal! k<CR>
    nmap <buffer> <C-j> :<C-u>normal! j<CR>
    nmap <buffer> ,r <plug>(molder-reload)
    nmap <buffer> ~ <plug>(molder-home)
    nmap <buffer> . <plug>(molder-toggle-hidden)

    nmap <buffer> gn :<C-u>call <SID>molder_newfile()<CR>
    ...
    " close
    nmap <buffer> q     :<C-u>call <SID>molder_close()<CR>
    nmap <buffer> <C-e> :<C-u>call <SID>molder_close()<CR>
endfunction
...
augroup my-ft-molder
    ...
    " 戻れるようにするため
    autocmd BufEnter * if &filetype !=# 'molder' | let w:molder_last_bufnr = bufnr() | endif
augroup END
